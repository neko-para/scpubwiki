...
export default {
  ...
  龙骑兵团: (p, c, g) => $()
    ...
    .bind('card-selled', () => 折跃(c, Array(c.locate('零件').length).fill('龙骑士'))),
  万叉奔腾: (p, c, g) => $()
    .for(c)
  ...
  暗影卫队: (p, c, g) => $()
    .for(c)
    .bind('post-enter', c.upgrade.push('暗影战士'))
    ...
    .bind('post-enter', () => p.asyncEnumPresent(async card => {
      if (card.race === 'P') {
        ...
        for (const i of idx.slice(0, g ? 2 : 1)) {
          const u = getUnit(card.unit[i])
          await 转换(card, [ i ], u.tag.includes('英雄单位') ? '英雄不朽者' : '不朽者')
        }
      }
    }))
  ...
  净化者军团: (p, c, g) => $()
    ...
    .bind('regroup', async () => {
      ...
      p.enumPresent(card => {
        if (card.race === 'P') {
          const us = card.locateX(unit => `${unit}(精英)` in data && !getUnit(unit).tag.includes('重型单位'))
            .map(i => card.unit[i])
          shuffle(us)
  ...
  凯拉克斯: (p, c, g) => $()
    .for(c)
    .bind('round-end', () => 折跃(c, Array(g ? 2 : 1).fill(
      shuffle([ '不朽者', '巨像', '掠夺者' ])[0]
    ))),
  虚空舰队: (p, c, g) => $()
    .for(c)
    .bind('round-end', 集结(c, 5, 0))
  ...
  黄金舰队: (p, c, g) => $()
    ...
    .bind('regroup-count', ({ info }) => info.count = 2),
  莫汗达尔: (p, c, g) => $()
    .for(c)
  ...
  菲尼克斯: (p, c, g) => $()
    .for(c)
    .bind('post-enter', () => 相邻两侧(async card => {
  ...
  晋升仪式: (p, c, g) => $()
    ...
    .bind('regroup', async () => {
      ...
      await 转换(c, c.locateX(u => getUnit(u).tag.includes('生物单位'), g ? 2 : 1), '高阶圣堂武士')
    }),
  英雄叉: (p, c, g) => $()
    .for(c)
