...
function 虫卵牌描述 (p, c, g) {
  return $()
    ...
    .bind('round-start', async () => {
      ...
      if (k === 2) {
        await 孵化(c, c.unit.filter(u => getUnit(u).tag.includes('生物单位')))
        await 摧毁(c)
        c.player.mineral += 1
      }
    })
...
export default {
  ...
  感染深渊: (p, c, g) => $()
    ...
    .bind('round-start', () => p.asyncEnumPresent(async card => 转换(card, card.locate('被感染的陆战队员', g ? 2 : 1), '畸变体'))),
  腐化大龙: (p, c, g) => $()
    .for(c)
    .bind('round-end', () => 转换(c, c.locate('腐化者', g ? 4 : 2), '巢虫领主'))
  ...
  终极进化: (p, c, g) => $()
    .for(c)
    .bind('post-enter', () => 相邻两侧(async card => 转换(card, card.locateX(u => ['蟑螂', '蟑螂(精英)'].includes(u), g ? 2 : 1), '莽兽'))),
  凶猛巨兽: (p, c, g) => $()
    .for(c)
    ...
    .bind('round-end', () => 相邻两侧(async card => {
      if (card.race === 'Z') {
  ...
  斯托科夫: (p, c, g) => $()
    ...
    .bind('card-enter-after$', async ({ card }) => {
      ...
      p.flag.斯托科夫E = 1
      if (p.flag.斯托科夫) {
        await 注卵(p.flag.斯托科夫C, card.unit.filter(u => {
          const uu = getUnit(u)
  ...
  守卫巢穴: (p, c, g) => $()
    ...
    .bind('round-end', async () => {
      ...
      await p.asyncEnumPresent(async card => 转换(card, card.locateX(u => ['异龙', '异龙(精英)'].includes(u), g ? 2 : 1), '守卫'))
    }),
  生化危机: (p, c, g) => $()
    .for(c)
    .bind('round-end', 科挂(c, 2, () => 注卵(c, [
  ...
  基因突变: (p, c, g) => {
    const f = () => 相邻两侧(async card => {
      if (card.race === 'Z') {
        const us = card.unit.map((u, i) => {
          const uu = getUnit(u)
          return {
            idx: i,
            unit: u,
        ...
        await 转换(card, us.slice(0, g ? 2 : 1).map(ui => ui.idx), u)
      }
    })
    return $()
      .for(c)
